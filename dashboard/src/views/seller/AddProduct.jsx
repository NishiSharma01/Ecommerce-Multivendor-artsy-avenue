...
const AddProduct = () => {
    ...
    const categorySearch = useCallback((e) => {
    ...
    const imageHandle = useCallback((e) => {
        ...
        if (length > 0) {
            ...
            setImageShow(prevImageShow => [...prevImageShow, ...imageUrl]);
        }
    }, []);

    useEffect(() => {
        return () => {
            imageShow.forEach(img => URL.revokeObjectURL(img.url));
        };
    }, [imageShow]);
    ...
    const changeImage = (img, index) => {
        if (img) {
            setImageShow(prevImageShow => {
                const newImageShow = [...prevImageShow];
    ...
    const removeImage = (i) => {
        ...
        setImageShow(prevImageShow => prevImageShow.filter((_, index) => index !== i));
    };
                        ...
                        <div className='grid lg:grid-cols-4 grid-cols-1 md:grid-cols-3 sm:grid-cols-2 sm:gap-4 md:gap-4 gap-3 w-full text-[#d0d2d6] mb-4'>
                            {
                                imageShow.map((img, i) => (
                                    <div key={i} className='h-[180px] relative'>
                                        <label htmlFor={i}>
                                            <img className='w-full h-full rounded-sm' src={img.url} alt={`Product preview ${i}`} />
                                        </label>
